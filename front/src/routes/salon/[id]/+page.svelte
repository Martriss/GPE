<script lang="ts">
    ...
    import { getGameTypeWithQueryDocumentSnapshot } from "$lib/utils/mapData";

    import type { PageProps } from "./$types";

    let { data }: PageProps = $props();

    // Reactive room data that will be updated by polling
    let roomData: GameType = $state(data.room);
    let pollingInterval: NodeJS.Timeout | null = null;

    const connectedUser: string = data.connectedUser;

    // Derived reactive values
    const roomName: string = $derived(roomData.roomName);
    const codeToJoin: string = $derived(roomData.code);
    const roomOwner: string = $derived(roomData.roomOwner);
    const amIRoomOwner: boolean = $derived(connectedUser === roomOwner);

    const isMe = (id: string): boolean => id === connectedUser;

    const shapePlayerList = () => {
        const array: InteractiveListItem[] = [];
        roomData.ruleset.players.forEach((player: PlayerType) => {
            const valueId: string = extractValueIdentityPlayer(player);
            if (valueId !== "") {
                const deck = roomData.decks.find(
                    (deck) => deck.playerId === valueId,
                );
                array.push({
                    key: valueId,
                    value: valueId,
    ...
    const shapeViewerList = () => {
        const array: InteractiveListItem[] = [];
        roomData.viewers.forEach((viewer: string) => {
            array.push({
                key: viewer,
                value: viewer,
        ...
        const j = roomData.viewers.findIndex(
            (viewer) => viewer === connectedUser,
        );
        if (i === -1 && j === -1) {
            array.push({
                key: connectedUser,
    ...
    // Update lists when roomData changes
    const updatePlayerLists = () => {
        players = shapePlayerList();
        viewers = shapeViewerList();
    };
    ...
    const nbPlayerMax: number = $derived(roomData.ruleset.players.length);
    ...
    const handleJoinPlayer = () => {
        ...
        try {
            roomData.ruleset.players = extractPlayersFromInteractiveListItem(
                players,
                roomData.ruleset.players,
            );
        } catch {
        ...
        roomData.viewers = extractValueFromInteractiveListItem(viewers);

        const response = fetch(`/api/games/${roomData.id}`, {
            method: "PUT",
    ...
    const actionsBeforeLeaveRoom = async () => {
        ...
        try {
            ...
            await fetch(`/api/games/${roomData.id}`, {
                method: "PUT",
                body: JSON.stringify(roomData),
                headers: {
                    "content-type": "application/json",
    ...
    const handleConfirmLeave = async () => {
        ...
        const response = await fetch(`/api/games/${roomData.id}`, {
            method: "DELETE",
        });
    ...
    const handleChooseUrDeck = async () => {
        ...
        const response = await fetch(
            `/api/decks/available-for-game?rulesetId=${roomData.ruleset.id}&minSizeDeck=${userPlayer.minSizeDeck}&maxSizeDeck=${userPlayer.maxSizeDeck}`,
            {
                method: "GET",
            },
        );
    ...
    const removeMyDeckInRoomDecks = () => {
        const i = roomData.decks.findIndex(
            (deck: SubDeckTypeForGame) => deck.playerId === connectedUser,
        );
        if (i !== -1) {
            const elSup = roomData.decks.splice(i, 1);
            return elSup[0];
        }
    };
    ...
    const handleStartGame = async () => {
        ...
        try {
            ...
            if (response.ok) {
                // Wait a moment to ensure Firebase is synchronized before redirecting
                await new Promise((resolve) => setTimeout(resolve, 1000));
                // Redirect to the game
                goto(`/game/${roomData.id}`);
            } else {
    ...
    // Initialize server-side polling synchronization
    onMount(() => {
        // Poll server every 2 seconds for room updates
        const pollRoomUpdates = async () => {
            try {
                const response = await fetch(
                    `/api/games/${roomData.id}/status`,
                    {
                        method: "GET",
                        credentials: "include",
                    },
                ...
                if (response.ok) {
                    ...
                    // Check if the game has started - if so, redirect to game
                    if (updatedRoom.isPlaying && !roomData.isPlaying) {
                        goto(`/game/${updatedRoom.id}`);
                        return;
                    }

                    // Update local room data if changed
                    const oldData = JSON.stringify(roomData);
                    const newData = JSON.stringify(updatedRoom);
        ...
        // Start polling
        pollingInterval = setInterval(pollRoomUpdates, 2000);
    });
